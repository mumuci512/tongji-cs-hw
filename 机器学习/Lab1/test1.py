...
# 改进的手写Ridge回归类
class ImprovedMyRidge:
    """
    改进的Ridge回归模型，使用自适应学习率
    """
    
    def __init__(self, alpha=0.1, learning_rate=0.01, n_iterations=1000, tolerance=1e-6):
        self.alpha = alpha
    ...
    def fit(self, X, y):
        ...
        for iteration in range(self.n_iterations):
            ...
            # 自适应学习率调整
            if iteration > 0:
                loss_improvement = prev_loss - total_loss
                if loss_improvement > 0:
                    # 损失在下降，可以适当增大学习率
                    adaptive_lr = min(adaptive_lr * 1.05, self.learning_rate * 10)
                else:
                    # 损失在上升，减小学习率
                    adaptive_lr = max(adaptive_lr * 0.5, self.learning_rate * 0.1)
            ...
            # 检查收敛
            if iteration > 10 and abs(self.loss_history[-1] - self.loss_history[-2]) < self.tolerance:
                break
...
# 改进的手动K折交叉验证
def improved_manual_kfold_cv(X_train, y_train, alphas, k=5):
    """
    改进的手动K折交叉验证，使用更稳定的优化方法
    """
    print(f"\n正在进行改进的手动 {k} 折交叉验证...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train), 1):
        ...
        for alpha in alphas:
            # 使用改进的Ridge模型
            model = ImprovedMyRidge(
                alpha=alpha, 
                learning_rate=0.001,  # 更小的学习率
                n_iterations=2000,    # 更多迭代次数
    ...
    # 找到最优alpha
    best_alpha_manual = min(alpha_avg_mse, key=alpha_avg_mse.get)
    best_mse_manual = alpha_avg_mse[best_alpha_manual]
    
    return best_alpha_manual, best_mse_manual, alpha_avg_mse

# 在automated_regression_pipeline函数中替换手动K折交叉验证部分
def improved_automated_regression_pipeline(X, y, target_index=0):
    """
    改进的自动化回归管道
    """
    # 选择目标变量
    target_name = y.columns[target_index]
    y_target = y.iloc[:, target_index]
    ...
    # 超参数搜索空间 - 更密集的搜索
    alphas = np.logspace(-4, 2, 100)  # 更多的alpha值
    
    # 1. 改进的手动K折交叉验证选择最优alpha
    print("\n🔍 改进的手动K折交叉验证选择最优alpha...")
    best_alpha_manual, best_mse_manual, alpha_mse_scores = improved_manual_kfold_cv(
        X_train_scaled, y_train, alphas, k=5
    )
    
    # 2. 使用GridSearchCV选择最优alpha
    ...
    # 分析差异原因
    print(f"\n🔍 差异分析:")
    manual_min_alpha = min(alpha_mse_scores.keys())
    manual_max_alpha = max(alpha_mse_scores.keys())
    print(f"  手动搜索的alpha范围: [{manual_min_alpha:.6f}, {manual_max_alpha:.6f}]")
    
    # 检查手动搜索的MSE曲线
    manual_mse_values = [alpha_mse_scores[alpha] for alpha in alphas]
    
    # 选择最终使用的alpha（综合考虑）
    if abs(best_alpha_manual - best_alpha_grid) / best_alpha_grid > 10:  # 差异超过10倍
        print("  ⚠️  手动和自动选择结果差异较大，建议以GridSearchCV为准")
        final_alpha = best_alpha_grid
    else:
        print("  ✅ 手动和自动选择结果较为一致")
        final_alpha = (best_alpha_manual + best_alpha_grid) / 2  # 取平均
    ...
    plt.semilogx(alpha_values, mse_values, 'b-', linewidth=2, label='手动K折CV')
    plt.axvline(best_alpha_manual, color='r', linestyle='--', 
                label=f'手动最优: {best_alpha_manual:.6f}')
    ...
    plt.semilogx(alphas, -grid_results['mean_test_score'], 'g-', linewidth=2, label='GridSearchCV')
    plt.axvline(best_alpha_grid, color='r', linestyle='--', 
                label=f'GridSearch最优: {best_alpha_grid:.6f}')
    plt.xlabel('Alpha (对数尺度)')
    plt.ylabel('平均MSE')
    ...
    plt.semilogx(alpha_values, mse_values, 'b-', linewidth=2, label='手动K折CV')
    plt.semilogx(alphas, -grid_results['mean_test_score'], 'g-', linewidth=2, label='GridSearchCV')
    plt.axvline(best_alpha_manual, color='blue', linestyle='--', alpha=0.7, label='手动最优')
    plt.axvline(best_alpha_grid, color='green', linestyle='--', alpha=0.7, label='GridSearch最优')
    plt.axvline(final_alpha, color='red', linestyle='-', linewidth=2, label=f'最终选择: {final_alpha:.6f}')
    plt.xlabel('Alpha (对数尺度)')
    plt.ylabel('平均MSE')
    plt.title('手动 vs GridSearchCV 对比')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    return results_df, best_models, X_test_scaled, y_test, target_name, scaler
...
def generate_final_report(results_df, target_name, coef_df):
    ...
    print(f"\n模型性能对比:")
    print(f"  Ridge Regression - R²: {ridge_result['R2']:.6f}, MSE: {ridge_result['MSE']:.6f}")
    print(f"  OLS Regression - R²: {ols_result['R2']:.6f}, MSE: {ols_result['MSE']:.6f}")
    ...
    if best_r2 > 0.9:
        ...
        print("  ⚠️  基本适合回归任务 - 模型有一定的预测能力，但还有改进空间")
    else:
    ...
    if ridge_result['R2'] > ols_result['R2']:
        print(f"  1. 主要模型: Ridge Regression (R² = {ridge_result['R2']:.4f})")
        print(f"  2. 备选模型: OLS Regression (R² = {ols_result['R2']:.4f})")
        advantage = "更好的泛化能力和对多重共线性的鲁棒性"
    else:
        print(f"  1. 主要模型: OLS Regression (R² = {ols_result['R2']:.4f})")
        print(f"  2. 备选模型: Ridge Regression (R² = {ridge_result['R2']:.4f})")
...
def main():
    ...
    for target_idx, target_name in enumerate(y.columns):
        ...
        # 运行回归管道
        results_df, best_models, X_test, y_test, target_name, scaler = improved_automated_regression_pipeline(
            X, y, target_idx
        )
        
        # 可视化分析
        coef_df = visualize_ridge_analysis(results_df, best_models, X_test, y_test, target_name, feature_names)
