...
{
	num = 2150000;
	strncpy(name, "<学生S>", strlen("<学生S>") + 1);
	sex = 'M';
	strncpy(addr, "四平路1239号", strlen("四平路1239号") + 1);
}
...
Student::Student(const int num1, const char* name1, const char sex1, const char* addr1)
{
	...
	if (strlen(name1) > 15)
	{
		...
		strncpy(name, name1, strlen(name1) + 1);
	sex = sex1;
	if (strlen(addr1) > 63)
	{
		...
		strncpy(addr, addr1, strlen(addr1) + 1);
}
...
{
	num = 21000;
	strncpy(name, "<教师T>", strlen("<教师T>") + 1);
	sex = 'M';
	strncpy(addr, "四平路1239号衷和楼", strlen("四平路1239号衷和楼") + 1);
}
...
Student& Student::operator=(const Teacher& t)
{
	...
	strcpy(name,"学生");
	strncpy(name+strlen("学生"), t.name+strlen("学生"),sizeof(t.name)- strlen("学生"));

	sex = t.sex;

	strncpy(addr, t.addr, strlen(t.addr) + 1);
	strcat(addr, "101室");
...
{
	Teacher t;
	t.num = 21000 + num % 1000;

	strcpy(t.name,"教师");
	strncpy(t.name+strlen("教师"),name+strlen("教师"),sizeof(name)- strlen("教师"));

	t.sex = sex;

	strncpy(t.addr, addr, strlen(addr) + 1);
	strcat(t.addr, "电信学院");
