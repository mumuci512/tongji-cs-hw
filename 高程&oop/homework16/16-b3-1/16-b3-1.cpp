...
Teacher& Teacher::operator=(const Student& s)
{
	...
	strcpy(name, "教师");
	strncpy(name + strlen("教师"), s.name + strlen("教师"), sizeof(s.name) - strlen("教师"));

	sex = s.sex;

	strncpy(addr, s.addr, strlen(s.addr) + 1);
	strcat(addr, "电信学院");
...
Teacher::operator Student()
{
	...
	strcpy(s.name, "学生");
	strncpy(s.name + strlen("学生"), name+strlen("学生"), sizeof(name)-strlen("学生"));

	s.sex = sex;

	strncpy(s.addr, addr, strlen(addr) + 1);
	strcat(s.addr, "101室");
