...
{
	ifstream in(in_name, ios::in);//不加binary，将回车视为一个字符，便于处理。但文件中的0D 0A会被视为一个回车
	if (!in)
	{
	...
	ofstream out(out_name, ios::out | ios::binary);//demo输出的hex文件中的回车经过ctrl H后为0D 0A，说明out处不加binary
	if (!out)
	{
	...
	while (1) {
		if (c % LINE == 0)
		{
			if(c!=0)
			{
				...
				for (int i = 0; i < 4 + LINE + 1; i++, in.get())
					;
			}
			if (in.eof())//正好在文件末尾
				break;
			in.seekg(8 + 2, ios::cur);//跳过下一行开头的表示地址的数字（8位）+2个空格
		}
		else if (c % BYTE == 0 && c % LINE != 0)
			in.seekg(2, ios::cur);//跳过每八位就会有一个的‘-’+一个空格
