...
int main()
{
    ...
    cur_e = (ElemType)malloc(80 * sizeof(char));	//e1实际是指针，必须先申请相应的空间
    for (i=INSERT_NUM*2; i>0; i-=2) {
	sprintf(e1, "test%03d", i);
	ListInsert(&L, 1, e1);
	}
    ...
    for(int i=0;i<15;i++)
    {
        ...
        scanf("%s", cur_e);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
        printf("\n请输入在哪个元素前插入(学号 姓名 性别 成绩 地址)：\n");
        scanf("%d %s %c %f %s", &cur_e.num, cur_e.name, &cur_e.sex, &cur_e.score, cur_e.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("\n请输入在哪个元素前插入(学号 姓名 性别 成绩 地址)：\n");
        scanf("%d %s %c %f %s", &cur_e->num, cur_e->name, &cur_e->sex, &cur_e->score, cur_e->addr);
#else	//缺省当做int处理
...
#ifdef ELEMTYPE_IS_DOUBLE
            printf("在元素%lf前插入元素=%5.1f成功\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
            printf("在元素%s前插入元素=%s成功\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
            printf("在元素\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e.num, cur_e.name, cur_e.sex, cur_e.score, cur_e.addr, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
            printf("在元素\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e->num, cur_e->name, cur_e->sex, cur_e->score, cur_e->addr, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
...
#ifdef ELEMTYPE_IS_DOUBLE
            printf("在元素%lf前插入元素=%5.1f失败\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
            printf("在元素%s前插入元素=%s失败\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
            printf("在元素\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", cur_e.num, cur_e.name, cur_e.sex, cur_e.score, cur_e.addr, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
            printf("在元素\"%d.%s.%c.%f.%s\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", cur_e->num, cur_e->name, cur_e->sex, cur_e->score, cur_e->addr, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
            printf("在元素%d前插入元素=%d失败\n", cur_e, e1);
#endif
        printf("ListInsert1测试完毕\n");
}
...
#ifdef ELEMTYPE_IS_DOUBLE
        printf("删除第一个元素%5.1f成功\n", cur_e);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
        printf("删除第一个元素=%s成功\n", cur_e);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
        printf("删除第一个元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e.num, cur_e.name, cur_e.sex, cur_e.score, cur_e.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
        printf("删除第一个元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e->num, cur_e->name, cur_e->sex, cur_e->score, cur_e->addr);
#else	//缺省当做int处理
        printf("删除第一个元素=%d成功\n", cur_e);
