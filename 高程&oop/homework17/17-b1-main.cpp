/* 2350222 盛曦 计科 */
/* 2350218 陈佳怡 2351274 王亦玮 2353806 赵翌佟 */
...
int main()
{
	...
	if (1)
	{
		...
		cout << "s1应该是Hello，  实际输出：" << (s1.assign(s1.assign(s1.assign(s1.assign(s1.assign(s1)))))) << endl;
	...
	if (1)
	{
		...
		cout << "insert函数简单测试(TStringAdv类和TStringAdv类)" << endl;
	...
	{
		TStringAdv s1;
		const char * str2 = nullptr, * str3 = NULL, str4[] = "ji";

		cout << "insert函数简单测试简单测试(TStringAdv类和字符指针、字符数组、字符串常量)" << endl;
	...
	{
		TStringAdv s1;
		cout << "insert函数简单测试简单测试(TStringAdv类和字符常量)" << endl;

		/*插入字符*/
		s1 = "tong";
		s1.insert('j', 5);
		cout << "s1应为tongj，实际输出：" << s1 << endl;

		s1 = "tong";
		s1.insert('j', 1);
		cout << "s1应为jtong，实际输出：" << s1 << endl;

		s1 = "tong";
		s1.insert('j', 2);
		cout << "s1应为tjong，实际输出：" << s1 << endl;
	...
	if (1)
	{
		...
		cout << "insert函数综合测试(TStringAdv类和TStringAdv类、字符指针、字符数组、字符)" << endl;

		s = "1.";
		s.insert( s1 + str2 + s3 + str4 + s5 + c,3);
		cout << "s应该是1.可以清心也!，实际输出：" << s << endl;

		s = "2.";
		cout << "s应该是2.也可以清心!，实际输出：" << (s.insert(s5 + s1 + str2 + s3 + str4[0] + str4[1] + c,3)) << endl;

		s = nullptr;
		cout << "s应该是3.心也可以清!，实际输出：" << ((s = "3.").insert(str4 + s5 + s1.c_str() + str2 + s3 + c,3)) << endl;

		s = nullptr;
		s += '5';
		s += '.';
		s1 = "可";
		cout << "s应该是5.以清心也可!，实际输出：" << (s.insert(str2,3).insert(s3,5).insert( str4,7).insert(s5.c_str(),9).insert(s1.c_str(),11).insert(c,13)) << endl;
	...
	if (1) {
		...
		cout << "erase函数简单测试(TStringAdv类和TStringAdv类、字符指针、字符数组、字符)" << endl;
	...
	if (1) {
		...
		cout << "erase函数综合测试(TStringAdv类和TStringAdv类、字符指针、字符数组、字符)" << endl;
	...
	if (1)
	{
		...
		s1 = "tong";
		cout << "s1应为tong，  实际输出：" << s1.substr(1) << endl;
		cout << "s1应为tong，  实际输出：" << s1.substr(1, 4) << endl;
		cout << "s1应为ong，   实际输出：" << s1.substr(2) << endl;
		cout << "s1应为on，    实际输出：" << s1.substr(2, 2) << endl;
		cout << "s1应为ng，    实际输出：" << s1.substr(3) << endl;
		cout << "s1应为n，     实际输出：" << s1.substr(3, 1) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(5) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(5, 1) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(0) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(0, 1) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(1, 0) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(1,-1) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << s1.substr(5, 1) << endl;
		cout << "s1应为tong，  实际输出：" << s1.substr(1, 12345) << endl;

		TStringAdv s2;
		cout << "s1应为tong，  实际输出：" << (s2 = s1.substr(1)) << endl;
		cout << "s1应为tong，  实际输出：" << (s2 = s1.substr(1, 4)) << endl;
		cout << "s1应为ong，   实际输出：" << (s2 = s1.substr(2)) << endl;
		cout << "s1应为on，    实际输出：" << (s2 = s1.substr(2, 2)) << endl;
		cout << "s1应为ng，    实际输出：" << (s2 = s1.substr(3)) << endl;
		cout << "s1应为n，     实际输出：" << (s2 = s1.substr(3, 1)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(5)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(5, 1)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(0)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(0, 1)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(1, 0)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(1, -1)) << endl;
		cout << "s1应为<EMPTY>，实际输出：" << (s2 = s1.substr(5, 1)) << endl;
		cout << "s1应为tong，  实际输出：" << (s2 = s1.substr(1, 12345)) << endl;
	...
	if (1) {
		...
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s3 = s1 + (s2 = "ji")) << endl;
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s3 = s1 + "ji") << endl;
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s2 = s1 + "ji") << endl;
		cout << "串为tong，     长度应为4， 实际：" << TStringAdvLen(s3 = s1 + nullptr) << endl;

		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s3 = (s1 += (s2 = "ji"))) << endl;
		cout << "串为tongji，   长度应为8， 实际：" << TStringAdvLen(s3 = (s1 += "ji")) << endl;
		cout << "串为tongji，   长度应为12，实际：" << TStringAdvLen(s3 = (s2 += s1 + "ji")) << endl;
