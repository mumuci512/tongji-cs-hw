/* 2353806 赵翌佟 计科 */
/*2350222 盛曦 2350218 陈佳怡 2351274 王亦玮*/
...
int main()
{
	...
	if (1) {
		...
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s0 = s1 + (s2 = "ji")) << endl;
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s0 = s1 + "ji") << endl;
		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s2 = s1 + "ji") << endl;
		cout << "串为tong，     长度应为4， 实际：" << TStringAdvLen(s0 = s1 + nullptr) << endl;

		cout << "串为tongji，   长度应为6， 实际：" << TStringAdvLen(s0 = (s1 += (s2 = "ji"))) << endl;
		cout << "串为tongji，   长度应为8， 实际：" << TStringAdvLen(s0 = (s1 += "ji")) << endl;
		cout << "串为tongji，   长度应为12，实际：" << TStringAdvLen(s0 = (s2 += s1 + "ji")) << endl;
	...
	{
		TStringAdv s1("hello");
		TStringAdv s2("Tongji");
		TStringAdv s3;
		//测试assign的TStringAdv类型操作
		cout << "s1的初始值为：" << s1.c_str();
		s1.assign(s2);
		cout<<"，assign s2 操作后值为：" << s1 << endl;
		//测试assign的const char*类型操作
		cout << "s3的初始值为：" << s3;
		s3.assign("oop_tset");
		cout<<"，assign 字符串“oop_test”操作后值为：" << s3 << endl;
		//测试assign的empty const char*操作
		cout << "s3的初始值为：" << s3;
		s3.assign("");
		cout<< "，assign <EMPTY> 操作后的值为：" << s3 << endl;
		//测试assign的self-assign操作
		cout << "s2的初始值为" << s2;
		s2.assign(s2);
		cout<<"，assign s2 操作后的值为：" << s2<< endl;
	...
	if (1)
	{
		...
		// 测试 append 的 TStringAdv 操作
		cout << "s1的初始值为：" << s1.c_str();
		s1.append(s2);
		cout << "，append s2 操作后值为：" << s1.c_str() << endl;

		// 测试 append 的 const char* 操作
		cout << "s3的初始值为：" << s3.c_str();
		s3.append("oop_test");
		cout << "，append 字符串 “oop_test” 操作后值为：" << s3.c_str() << endl;

		// 测试 append 的空字符操作
		cout << "s3的初始值为：" << s3.c_str();
		s3.append("");
		cout << "，append 空字符串操作后值为：" << s3.c_str() << endl;

		// 测试 append 的单个字符操作
		cout << "s1的初始值为：" << s1.c_str();
		s1.append('!');
		cout << "，append 单个字符“！”操作后值为：" << s1.c_str() << endl;
	...
	if (1)
	{
		...
		// 测试插入 TStringAdv 对象
		cout << "s1 的初始值为：" << s1.c_str() << endl;
		s1.insert(s2, 3);
		cout << "在位置3插入 s2 后 s1 的期望值为：heTongjillo，实际值为：" << s1.c_str() << endl;
		cout << endl;
		// 重置 s1
		s1.assign("hello");

		// 测试插入 const char* 字符串
		cout << "s3 的初始值为：" << s3<< endl;
		s3.insert("oop_test", 1);
		cout << "在位置1插入 \"oop_test\" 后 s3 的期望值为：oop_test，实际值为：" << s3.c_str() << endl;
		cout << endl;
		// 测试插入空字符串
		cout << "在位置1插入空字符串前 s3 的初始值为：" << s3.c_str() << endl;
		s3.insert("", 1);
		cout << "在位置1插入空字符串后 s3 的期望值为：oop_test，实际值为：" << s3.c_str() << endl;  // 不变
		cout << endl;
		// 插入 const char* 到中间位置
		cout << "s1 的初始值为：" << s1.c_str() << endl;
		s1.insert("world", 3);
		cout << "在位置3插入 \"world\" 后 s1 的期望值为：heworldllo，实际值为：" << s1.c_str() << endl;
		cout << endl;
		// 重置 s1
		s1.assign("hello");
		...
		cout << "s1 的初始值为：" << s1.c_str() << endl;
		s1.insert('!', 6);
		cout << "在位置6插入字符 '!' 后 s1 的期望值为：hello!，实际值为：" << s1.c_str() << endl;  // hello!
		cout << endl;
		// 测试插入字符到尾部
		cout << "s2 的初始值为：" << s2.c_str() << endl;
		s2.insert('_', s2.length() + 1);  // 插入到尾部
		cout << "在尾部插入字符 '_' 后 s2 的期望值为：Tongji_，实际值为：" << s2.c_str() << endl;  // Tongji_
		cout << endl;
		// 重置 s2
		s2.assign("Tongji");

		// 测试非法位置插入字符
		cout << "尝试在非法位置插入字符前 s2 的初始值为：" << s2.c_str() << endl;
		s2.insert('!', -1);  // 非法位置
		cout << "在非法位置插入字符后 s2 的期望值为：Tongji，实际值为：" << s2.c_str() << endl;  // 不变
		cout << endl;
		// 插入在大于尾部位置的字符（非法位置）
		cout << "尝试在大于尾部的位置插入字符前 s2 的初始值为：" << s2.c_str() << endl;
		s2.insert('!', s2.length() + 10);  // 非法位置
		cout << "在大于尾部位置插入字符后 s2 的期望值为：Tongji，实际值为：" << s2.c_str() << endl;  // 不变
		cout << endl;
	...
	if (1)
	{
		TStringAdv s1("hello_world");
		TStringAdv s2("world");
		TStringAdv s3("oop_test");

		// 测试 erase 的 TStringAdv 类型操作
		cout << "初始值 s1：" << s1.c_str();
		s1.erase(s2);
		cout << "期望值 s1：hello_，实际值 s1：" << s1.c_str() << endl;

		// 重置 s1
		s1.assign("hello_world");

		// 测试 erase 的 const char* 类型操作
		cout << "初始值 s1：" << s1.c_str();
		s1.erase("world");
		cout << "期望值 s1：hello_，实际值 s1：" << s1.c_str() << endl;

		// 测试 erase 的单字符操作
		cout << "初始值 s3：" << s3.c_str();
		s3.erase('o');
		cout << "期望值 s3：op_test，实际值 s3：" << s3.c_str() << endl;

		// 测试空内容删除
		TStringAdv s4;
		cout << "初始值 s4：<EMPTY> " << s4.c_str();
		s4.erase("anything");
		cout << "期望值 s4：<EMPTY>，实际值 s4：" << s4.c_str() << endl;

		// 测试不存在的内容删除
		cout << "初始值 s1：" << s1.c_str();
		s1.erase("not_in_string");
		cout << "期望值 s1：hello_，实际值 s1：" << s1.c_str() << endl;

		// 测试自定义字符删除
		cout << "初始值 s1：" << s1.c_str();
		s1.erase('_');
		cout << "期望值 s1：hello，实际值 s1：" << s1.c_str() << endl;
		...
		s3.assign("abacadae");
		s3.erase('a');
		cout << "期望值 s3：bacadae，实际值 s3：" << s3.c_str() << endl;
	...
	if (1)
	{
		...
		// 测试 substr 从位置 1 开始，长度为 5
		TStringAdv substr1 = s1.substr(1, 5);
		cout << "从位置1开始，长度为5 的子串期望值为：hello，实际值为：" << substr1.c_str() << endl;
		cout << endl;

		// 测试 substr 从位置 7 开始，长度为 5
		TStringAdv substr3 = s1.substr(7, 5);
		cout << "从位置7开始，长度为5 的子串期望值为：world，实际值为：" << substr3.c_str() << endl;
		cout << endl;

		// 测试 substr 从位置 6 开始，缺省长度
		TStringAdv substr4 = s1.substr(6);
		cout << "从位置6开始，不带长度(缺省)的子串期望值为：_world，实际值为：" << substr4.c_str() << endl;
		cout << endl;

		// 测试越界的 substr 调用
		TStringAdv substr5 = s1.substr(-1, 3);
		cout << "从非法位置开始的子串期望值为空字符串，实际值为：<" << substr5.c_str() << ">" << endl;
		cout << endl;

		// 测试长度为 0 的 substr 调用
		TStringAdv substr6 = s1.substr(4, 0);
		cout << "从位置4开始，长度为0的子串期望值为空字符串，实际值为：<" << substr6.c_str() << ">" << endl;
		cout << endl;
	...
	if (1)
	{
		...
		try {
			char& ch = s1.at(0);
			cout << "位置0的字符期望值为：h，实际值为：" << ch << endl;
		}
		catch (const std::out_of_range& e) {
			cout << "捕获越界异常：" << e.what() << endl;
		}
		cout << endl;
		...
		try {
			char& ch = s1.at(-1);
			cout << "位置-1的字符期望抛出异常，实际值为：" << ch << endl;
		}
		catch (const std::out_of_range& e) {
			cout << "位置-1的字符期望抛出异常，捕获越界异常：" << e.what() << endl;
		}
		cout << endl;

		try {
			char& ch = s1.at(11);
			cout << "位置11的字符期望抛出异常，实际值为：" << ch << endl;
		}
		catch (const std::out_of_range& e) {
			cout << "位置11的字符期望抛出异常，捕获越界异常：" << e.what() << endl;
		}
		cout << endl;
