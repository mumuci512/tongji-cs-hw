...
int main()
{
	if (1) {
		...
		cout << "赋值函数测试" << endl;
		s3.assign(s1);
		cout << "s3.assign(s1): 结果应为Hello，实际：" << s3 << endl;
		s3.assign("Test");
		cout << "s3.assign(\"Test\"): 结果应为Test，实际：" << s3 << endl;

		cout << "追加函数测试" << endl;
		s1.append(s2);
		cout << "s1.append(s2): 结果应为HelloWorld，实际：" << s1 << endl;
		s1.append("!");
		cout << "s1.append(\"!\"): 结果应为HelloWorld!，实际：" << s1 << endl;
		s1.append('?');
		cout << "s1.append('?'): 结果应为HelloWorld!?，实际：" << s1 << endl;

		cout << "插入函数测试" << endl;
		s1.insert(s2, 6);
		cout << "s1.insert(s2, 6): 结果应为HelloWorldWorld!?，实际：" << s1 << endl;
		s1.insert("Insert", 11);
		cout << "s1.insert(\"Insert\", 11): 结果应为HelloWorldInsertWorld!?，实际：" << s1 << endl;
		s1.insert('@', 1);
		cout << "s1.insert('@', 1): 结果应为@HelloWorldInsertWorld!?，实际：" << s1 << endl;

		cout << "删除函数测试" << endl;
		s1.erase(s2);
		cout << "s1.erase(s2): 结果应为@HelloInsertWorld!?，实际：" << s1 << endl;
		s1.erase("Insert");
		cout << "s1.erase(\"Insert\"): 结果应为@HelloWorld!?，实际：" << s1 << endl;
		s1.erase('@');
		cout << "s1.erase('@'): 结果应为HelloWorld!?，实际：" << s1 << endl;

		cout << "子串函数测试" << endl;
		TStringAdv sub = s1.substr(6, 5);
		cout << "s1.substr(6, 5): 结果应为World，实际：" << sub << endl;
		sub = s1.substr(11);
		cout << "s1.substr(11): 结果应为!?，实际：" << sub << endl;

		cout << "访问函数测试" << endl;
		char c = s1.at(0);
		cout << "s1.at(0): 结果应为H，实际：" << c << endl;
		s1.at(0) = 'h';
		cout << "s1.at(0) = 'h': 结果应为helloWorld!?，实际：" << s1 << endl;

		cout << "长度函数测试" << endl;
		int len = TStringAdvLen(s1);
		cout << "TStringAdvLen(s1): 结果应为12，实际：" << len << endl;
