...
void fall(struct magic_ball& magic, int& score, int game, const CONSOLE_GRAPHICS_INFO* const cgi, BLOCK_DISPLAY_INFO* bdi)//下落除0
{
	int i, j, k;
	for (j = 0; j < magic.x; j++)
	{
			...
			if (flag == 0)
			{
				for (k = j; k <magic.x-1; k++)
				{
					for (i = magic.y - 1; i >= 0; i--)//因为是整列移动，无法用行指针，若数组长宽相反则可以
					{
						...
						gmw_move_block(cgi, k, j, magic.ball[k+1][i], 0, bdi, RIGHT_TO_LEFT, 1);
					}
				}
			}
		}
