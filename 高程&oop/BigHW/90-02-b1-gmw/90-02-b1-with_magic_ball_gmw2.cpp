...
void fall(struct magic_ball& magic, int& score, const CONSOLE_GRAPHICS_INFO* const cgi,const BLOCK_DISPLAY_INFO* const bdi)//下落除0
{
	int i, j, k;
	for (j = 0; j < magic.x; j++)
	{
	...
	for (j = 0; j < magic.x; j++)
	{
		for (i = magic.y - 1; i >= 0; i--)
		{
			if (flag == 1 && magic.ball[i][j] != 0 && magic.ball[i + 1][j] == 0)//当前不为0，当前位置的下方一个为0，这时候才需要下落；因为刚进循环的时候flag为0，不可能进这个if，所以横坐标i+1不会越界
			{
				...
				{
					gmw_move_block(cgi, k, j, magic.ball[k][j], 0, bdi, UP_TO_DOWN, 1);
			...
			for (k = j; k < magic.x - 1; k++)
			{
				for (i = magic.y - 1; i >= 0; i--)//因为是整列移动，无法用行指针，若数组长宽相反则可以
				{
					if (magic.ball[i][k + 1] == 0)
						break;
					gmw_move_block(cgi, i, k + 1, magic.ball[i][k + 1], 0, bdi, RIGHT_TO_LEFT, 1);
...
void print_ball(struct magic_ball& magic,const BLOCK_DISPLAY_INFO* const bdi, const BLOCK_DISPLAY_INFO* const bdi_sel,const CONSOLE_GRAPHICS_INFO* const cgi)
{
	cct_setcursor(CURSOR_INVISIBLE);
	struct location loc;
...
void cancel_previous(struct magic_ball& magic, const CONSOLE_GRAPHICS_INFO* const cgi, const BLOCK_DISPLAY_INFO* const bdi)
{
	struct location loc;
	for (loc.i = 0; loc.i < magic.y; loc.i++)
	{
