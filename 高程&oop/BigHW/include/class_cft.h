...
private:
	...
	vector<string> group_name_list; //存储配置文件中的所有组名
	vector<vector<string>> item_list; //存储配置文件中的所有项（一整行）,去除了项目名前和值之后的空格
	vector<vector<string>> item_name_list; //存储配置文件中的所有项名，去除了项目名前后的空格
	vector<vector<string>> item_content_list; //存储配置文件中的所有项值，去除了项目值前后的空格
	bool is_line_legal; //标记是否有超出指定长度的行
	bool is_open_file_success;//标记是否成功打开文件
...
public:
	...
	friend int find_group(config_file_tools* const cft, const char* const  group_name, const bool is_case_sensitive, vector<string>::iterator& p);
	friend int find_item(config_file_tools* const cft, const char* const item_name, const bool is_case_sensitive,const int group_index, vector<string>::iterator& q);
	friend int find_group_item(config_file_tools* const cft, const char* const group_name, const char* const item_name, const bool group_is_case_sensitive, const bool item_is_case_sensitive, int& group_index, int& item_index);
};
